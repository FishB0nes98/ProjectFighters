...
class FarmStage1(Stage):
    ...
    def load_resources(self):
        ...
        self.background = pygame.image.load("Raids/Farm Raid/res/Stage_1_BG.jpeg")
        ...
        self.player = Character("Farmer Cham Cham", 8725, "Loading Screen/Farmer Cham Cham.png", (player_x, player_y))
        
        # Create boss (Angry Carrot)
        self.boss = Character("Angry Carrot", 12500, "Raids/Farm Raid/res/Angry_Carrot.jpeg", (boss_x, boss_y))
    ...
    def setup_abilities(self):
        ...
        carrot_abilities = [
            ("Root Strike", "Raids/Farm Raid/res/Angry_Carrot_Icon.png", 370, 0, 0, True),
            ("Go Dormant", "Raids/Farm Raid/res/Angry_Carrot_Dormant_Buff.jpeg", 0, 3500, 8, False),
            ("Anger", "Raids/Farm Raid/res/Angry_Carrot_Anger_Buff.jpeg", 0, 0, 8, False),
            ("Carrot Grenade", "Raids/Farm Raid/res/Angry_Carrot_Icon.png", 430, 0, 5, True)
        ]
    ...
    def add_items_to_inventory(self):
        health_potion = Item("Health Potion", "Raids/Farm Raid/res/Health_Potion.jpeg")
    ...
    def handle_events(self, events):
        for event in events:
            ...
            if hasattr(self, 'show_victory_screen') and self.show_victory_screen:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if hasattr(self, 'next_stage_button') and self.nextstage_button.collidepoint(event.pos):
                        self.completed = True
                        self.running = False
                        return True
    ...
    def handle_inventory_clicks(self, pos):
        for slot in self.inventory.slots:
            if slot.rect.collidepoint(pos) and slot.item:
                # Use the item through the base class method
                if self.use_item(slot.item):
                    # Remove item if successfully used
                    slot.item = None
                    return True
        return False
    ...
    def spawn_mini_bosses(self):
        ...
        for i in range(3):
            ...
            mini_boss = Character(
                ...
                "Raids/Farm Raid/res/Angry_Carrot.jpeg",
                (x_pos, 50)
            )
    ...
    # Add a method to check if a character is dormant
    def is_dormant(self, character):
        for buff in character.buffs:
            if buff.effect_type == "dormant":
                return True
        return False
        ...
        if self.boss:
            if self.boss.current_hp > 0:
                self.targets.append(Target(
                    self.boss,
                    "Raids/Farm Raid/res/Angry_Carrot_Icon.png",
            ...
            for i, mini_boss in enumerate(self.mini_bosses):
                if mini_boss.current_hp > 0:
                    ...
                    self.targets.append(Target(
                        mini_boss,
                        "Raids/Farm Raid/res/Angry_Carrot_Icon.png",  # You might want to create a mini carrot icon
                        (target_x, target_y)
                    ))
